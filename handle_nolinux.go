...
func getHandle(fn string) (*handle, error) {
	...
	h := &handle{
		fn:  fn,
		dev: uint64(stat.Dev), //nolint:unconvert,nolintlint,gosec
		ino: uint64(stat.Ino), //nolint:unconvert,nolintlint,gosec
	}
...
func (h *handle) Path() (string, error) {
	...
	if uint64(stat.Dev) != h.dev || uint64(stat.Ino) != h.ino { //nolint:unconvert,nolintlint,gosec
